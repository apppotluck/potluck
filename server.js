...
apiRoutes.get('/events/getInvitees/:eId', function(req, res) {
    dbOperation.getEventInvitees(req.params.eId).then(function(invittesDetails) {
        res.json(invittesDetails);
        res.send();
    }, function(err) {
        res.json("error");
        res.send();
